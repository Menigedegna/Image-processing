...
#    <CustomTools>
#      <Menu>
#       <Item name="NucleiPloidyPollen2" icon="Python" tooltip="NucleiPloidyPollen2">
#         <Command>PythonXT::XTNucleiPloidyPollen2(%i)</Command>
#       </Item>
#      </Menu>
#    </CustomTools>
...
def CreateDirectoryToSaveFiles(Result_pathway):
    if os.path.exists(Result_pathway):
        ResultPathList=[]
        for i in range(3):
            Result_Folder          =           os.path.join(Result_pathway, "Result"+str(i))
            os.makedirs(Result_Folder)
            ResultPathList.append(Result_Folder)
    else:
        quit()
...
def ParallelTask(Image_folder, ResultPath):
...
#==============================================================================
            ...
            vImage               =   	vImaris.GetDataSet()
            if vImage is not None :
                GetImageFeatures(FileIndex, ResultPath, vFileName, vFullFileName)
                FileIndex	               +=  		1
...
# Process images
#==============================================================================
def XTNucleiPloidyPollen2(aImarisId):
    logging.basicConfig(level=logging.DEBUG, filename= "log[XTNucleiPloidyPollen2].log")
    try:
...
#==============================================================================
            ...
            root1.destroy()
#            Result_pathway          =           os.path.join(Image_folder, "XTNucleiPloidyPollen2_Result")
            ResultPathList          =         CreateDirectoryToSaveFiles(Result_pathway)
            ImageFolderList=[os.path.join(Image_folder, "ImageFolder"+str(x)) for x in range(3)]
            t1 = threading.Thread(target=ParallelTask(ImageFolderList[0], ResultPathList[0]))
            t2 = threading.Thread(target=ParallelTask(ImageFolderList[1], ResultPathList[1]))
            t3 = threading.Thread(target=ParallelTask(ImageFolderList[2], ResultPathList[2]))        
            t1.start()
            t2.start()
            t3.start()      
            t1.join()
